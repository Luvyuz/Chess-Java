...
public class Pawn extends Piece {
	...
	//se non ha ancora mosso puo muoversi di 2, altrimenti di 1. Puo catturare solo in diagonale di 1
	@Override
	public boolean makeMove(int toX, int toY, Board board) {
		...
		if(moves.contains(move)){
			//Cattura pezzo in diagonale destra
			if(toX == xCord + 1 && yCord -(isWhite ? 1 : -1) == toY) 
				if(board.getXY(toX, toY) == 0){
					//board.addDeadPieces(board.getPiece(xCord + 1, yCord));
			...
			//Cattura pezzo in diagonale sinistra
			if(toX == xCord -1 && yCord -(isWhite ? 1 : -1) == toY)
				if(board.getXY(toX, toY) == 0){
					//board.addDeadPieces(board.getPiece(xCord - 1, yCord));
					game.getAllPieces().remove(board.getPiece(xCord - 1, yCord));
			...
			//Prima mossa, quindi puo muoversi di 2
	...
	public boolean canMove(int x, int y, Board board){	
		...
		//controlla se qualcosa blocca la strada
		if((board.getPiece(x, y) != null && board.getPiece(x, y).isWhite() == isWhite()))
			return false;
		//puo muoversi in diagonale solo per catturare
		if(xCord != x && board.getPiece(x, y) == null)
			return false;
		if(isWhite){
			//puo moversi di 1 o di 2
			if(firstMove) 
